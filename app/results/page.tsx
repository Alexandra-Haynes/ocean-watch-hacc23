...
import { Cell, Label, Legend, Pie, PieChart, ResponsiveContainer, Text, Tooltip } from 'recharts';
...
export default function ResultsPage() {
  ...
  function transformRemovalJobsData(jobs: RemovalJobs[]): DebrisTypeSummary[] {
    return jobs.reduce((acc: DebrisTypeSummary[], job: RemovalJobs) => {
      ...
      const counts = jobs.reduce((acc: Record<string, number>, job: RemovalJobs) => {
  ...
  const calculateFishingGearPercentage = (data: RemovalJobs[]) => {
    ...
    const fishingGearCount = data.filter(job => job.debrisType === "A mass of netting and/or fishing gear").length;
    const fishingGearPercentage = parseFloat(((fishingGearCount / totalCount) * 100).toFixed(2));
    const otherPercentage = parseFloat(((100 - fishingGearPercentage).toFixed(2)));
  ...
  return (
    ...
    custom-background">
      ...
      <div className="flex justify-around">
        ...
        <div className="flex flex-col w-1/3">
          <h2 className="text-white justify-center m-auto">% Fishing Gear vs Other</h2>
          <ResponsiveContainer width="100%" height={400}>
          <PieChart>
            <Tooltip />
            <Legend layout="vertical" verticalAlign="bottom" align="center" />
