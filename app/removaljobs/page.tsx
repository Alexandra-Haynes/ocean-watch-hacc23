          ...
          .map((job, index) => {
            return (
              <div
                ...
                key={index}
                className="w-fit border-solid border-gray border-2 rounded-sm cursor-pointer bg-gray-50 flex flex-col w-250"
              >
                <h1 className="text-black self-center font-bold">
                  Job #{job.id}
                </h1>
                <h2 className="text-black font-bold">Debris Info</h2>
                <p className="text-black">Address - {job.address}</p>
                <p className="text-black">Latitude - {job.latitude}</p>
                <p className="text-black">Longitude - {job.longitude}</p>
                <p className="text-black">Date - {job.date}</p>
                <p className="text-black">Debris Type - {job.debrisType}</p>
                <p className="text-black">
                  Container Status - {job.containerStatus}
                </p>
        ...
        <form onSubmit={handleRemovalJobClaimSubmit} ref={modalRef}>
          <div className="bg-gray-50 flex flex-col fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-full w-1/3 h-1/3">
            ...
            <input
              ...
              onChange={handleRemovalJobClaim}
            />
